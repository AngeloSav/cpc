...
\begin{itemize}
    \item \textbf{is\_bst} is true if both the left and right subtrees are BSTs \textbf{and} if the max value in the left subtree is less or equal than the key in the node of node\_id \textbf{and} if the min value in the right subtree is greater than the key in the node of node\_id, otherwise is set to false.
